...
class VAE(nn.Module):
    def __init__(self, input_channels=3, latent_size=2, output_channels=3):
        ...
        # Encoder layers
        self.encoder_conv1 = nn.Conv2d(input_channels, 64, kernel_size=4, stride=2, padding=1)
        self.encoder_conv2 = nn.Conv2d(64, 128, kernel_size=4, stride=2, padding=1)
        self.encoder_conv3 = nn.Conv2d(128, 256, kernel_size=4, stride=2, padding=1)
        ...
        self.decoder_deconv3 = nn.ConvTranspose2d(64, output_channels, kernel_size=4, stride=2, padding=1)
    ...
    def encode(self, x):
        x = self.convs(x)
        encoding_shape = x.shape
        x = F.relu(self.upsample(x.view(x.size(0), x.size(1), -1)))
        mu, logvar = self.gaussian_parameters(x.view(x.size(0), -1), dim=1)
        return mu, logvar, encoding_shape
    ...
    def decode(self, z):
        ...
        reconstructed = torch.sigmoid(self.decoder_deconv3(z))
        return reconstructed

    def forward(self, x):
        mu, logvar, encoding_shape = self.encode(x)
        z = self.reparameterize(mu, logvar)
        reconstructed = self.decode(z.view(encoding_shape))
...
class WaveNet_VAE(nn.Module):
    """ USCT_U Network """

    def __init__(self, encode_layers, decode_layers, enc_dims=None, dec_dims=None,
                 input_channels=3, output_channels=3, vae_dims=None,
                 transitions=None, enc_mlp_ratios=None, dec_mlp_ratios=None,
                 height=9, norm_layer=LayerNorm, use_asff=True):

        super().__init__()
        self.wave_net = WaveNet(encode_layers, decode_layers, enc_dims, dec_dims, transitions=transitions,
                        enc_mlp_ratios=enc_mlp_ratios, dec_mlp_ratios=dec_mlp_ratios, height=height, norm_layer=norm_layer,
                        use_asff=use_asff)
        self.vae = VAE(input_channels, vae_dims, output_channels)

    def forward(self, x):
        wave_net_out = self.wave_net(x)
        vae_out, _, _ = self.vae(x)
        return wave_net_out + vae_out


@register_model
def WaveNet_T_VAE(pretrained=False, **kwargs):
    transitions = [True]
    encode_layers = [2]
    decode_layers = [2]
    enc_mlp_ratios = [1]
    ...
    model = WaveNet_VAE(encode_layers, decode_layers, enc_dims, dec_dims, input_channels, output_channels, vae_dims,
                    transitions=transitions, enc_mlp_ratios=enc_mlp_ratios, dec_mlp_ratios=dec_mlp_ratios,
                    height=9, norm_layer=LayerNorm, use_asff=use_asff, **kwargs)
    return model
